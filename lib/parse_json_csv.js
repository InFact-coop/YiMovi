...
const pathToJson = locale => path.join('locales', `/${locale}.json`);

const getFileData = locale =>
  JSON.parse(fs.readFileSync(pathToJson(locale)));

const flattenArrKeys = arr => _.flatten(
  _.keys(arr)
  .map(key => _.keys(arr[key]).map(innerKey => `${key}.${innerKey}`))
);
...
const createCsvFile = string => {
  const fileName = `translations_${_.now()}.csv`;
  const filePath = path.join('translation_csv', fileName);

  fs.writeFile(filePath, string, (err) => {
    if (err) return console.log(err);
    console.log(`success - new file created - ${fileName}`);
  });
};
